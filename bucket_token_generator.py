...
def print_formatted_token(token_dict: dict, token_format: str, platform: str = 'linux') -> None:
    if token_format == 'json':
        print(json.dumps(token_dict))
    elif token_format == 'text':
        if platform == 'windows':
            token_txt = "\n".join([f'set {key}={value}' for key, value in token_dict.items()])
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--platform', type=str, default='linux', help='Output platform (linux or windows)')
    ...
    try:
        ...
        print_formatted_token(token_dict, args.token_format, args.platform)
    except Exception as e:
        print(f"Error: {str(e)}")